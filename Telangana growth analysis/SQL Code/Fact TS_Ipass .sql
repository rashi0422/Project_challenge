...
with Output1 as 
  (select district,round(sum(investment_in_cr),2) as 'Investment_in_cr'
   from fact_ts_ipass f 
   join dim_date d 
   on f.month = d.month
   join dim_districts di 
   ...
   group by district),
output2 as 
  (select district, sum(estamps_challans_rev/1000000) as 'estamps_revenue'
   from fact_stamps f
...
output3 as 
  ( select district, round(sum(fuel_type_petrol + fuel_type_diesel + fuel_type_electric + fuel_type_others),2) as 'vechicle_revenue'
...
select sector,count(distinct district) as 'districts', 
